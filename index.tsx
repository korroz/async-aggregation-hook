...
import './style.css';
import { useAgg, calculate, useCalculationControl, useCalcUnits2, useCalcByIndex, useCalcByMap } from './rxtest';
...
function App() {
  ...
  return <div>
    <button onClick={() => setQueries(queries === q1 ? q2 : q1)}>Switch</button>
    <button onClick={() => setKey(containerKey === 'wat' ? 'wot' : 'wat')}>Refresh</button>
    <button onClick={calculate} disabled={!manualModeOn}>Calculate</button>
    <label>Manual
      <input type='checkbox' name='manual' checked={manualModeOn} onChange={toggleManual} />
    </label>
    <div className='agg-wrapper' key={containerKey}>
      {queries.map((q, i) => <AggDisplay query={q} key={i} />)}
...
function AggDisplay({ query }: { query: string }) {
  ...
  return <div className='agg-div'>
    <div className='counter' onClick={() => setCount(count + 1)}>Count: {count}</div>
    <div className='title'>AGG DISPLAY</div>
    <div className='query'>{query}</div>
    <div className='result'>
      <meter min='0' max='100' low='40' value={result.toString()}></meter><br />
      {result.toString()}
    </div>
  </div>;
}
...
const calcUnits = [
  {
    ...
    evals: [['product', x => x.cat * x.sister], ['r', x => x.product / x.sister]],
  },
  {
    aggs: { count: 'sister', base1: 'doom', base2: 'brother' },
    evals: [['base', x => x.base1 + x.base2], ['r', x => x.count / x.base * 100]]
  }
];
function CalcDisplay() {
  ...
  return <div className='agg-div'>
    <div className='counter' onClick={() => { setCount(count + 1); calc1.setter(unit => { unit.tags = { myTag: 'My fine tag' }; })}}>Count: {count}</div>
...
function IndexDisplay() {
  const [count, setCount] = useState(0);
  const [woot, doom] = useCalcByIndex<number | string>([
    { aggs: ['doom', 'cat'], evalFn: ([doom, cat]) => doom + cat },
    { aggs: ['sister', 'brother'], evalFn: ([sister, brother]) => sister / (sister + brother) }
  ], '-');
  return <div className='agg-div'>
    <div className='counter' onClick={() => setCount(count + 1)}>Count: {count}</div>
    ...
    <div className='result'>{typeof doom === 'number' ? (doom * 100).toFixed(2) : doom} %</div>
  </div>;
}
function MapDisplay() {
  const [count, setCount] = useState(0);
  const [woot, doom] = useCalcByMap<number | string>([
    { aggs: { count: 'thousand', animal: 'dog' }, evalFn: ({ count, animal }) => count + animal },
    { aggs: { a: 'doom', b: 'cat', c: 'thousand' }, evalFn: ({ a, b, c}) => (a + b + c) / 3 }
  ], '...');
  return <div className='agg-div'>
    <div className='counter' onClick={() => setCount(count + 1)}>Count: {count}</div>
    ...
    <div className='result'>{typeof doom === 'number' ? doom.toFixed(2) : doom} AVG</div>
  </div>;
}
